<script>
  ...
  async function cargar() {
    ...
    try {
      const [resU, resP] = await Promise.all([
        fetch(`/api/usuarios?pagina=${pagina}&limite=${POR_PAGINA}&buscar=${buscar}`, { headers: headersJSON() }),
        fetch(`/api/perfiles?pagina=1&limite=100`, { headers: headersJSON() })
      ]);
  ...
  function abrirEditar(u) {
    ...
    imagenPreview = u.strImagenPerfil ? `/uploads/${u.strImagenPerfil}` : '';
              ...
              <td>
                <div class="avatar-tabla">
                  {#if u.strImagenPerfil}
                    <img src={`/uploads/${u.strImagenPerfil}`} alt={u.strNombreUsuario} />
                  {:else}
      ...
      <div class="detalle-avatar">
        <div class="avatar-grande">
          {#if usuarioSeleccionado.strImagenPerfil}
            <img src={`/uploads/${usuarioSeleccionado.strImagenPerfil}`} alt="avatar" />
          {:else}
