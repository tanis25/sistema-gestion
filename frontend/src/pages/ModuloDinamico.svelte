<script>
  ...
  let nombreModulo = $state('');
  let rutaActual = window.location.pathname;

  // Extraer nombre del módulo de la URL /modulo/NombreModulo
  $effect(() => {
    const partes = rutaActual.split('/modulo/');
    if (partes[1]) {
      nombreModulo = decodeURIComponent(partes[1]);
    }
  });
  ...
  let perms = $derived(getPermisos(permisosVal, esAdminVal, nombreModulo));

  // Datos de ejemplo para la tabla
  const datos = [
    { id: 1, nombre: 'Registro 001', descripcion: 'Descripción del registro 001', estado: 'Activo', fecha: '2026-01-15' },
    { id: 2, nombre: 'Registro 002', descripcion: 'Descripción del registro 002', estado: 'Activo', fecha: '2026-02-20' },
    { id: 3, nombre: 'Registro 003', descripcion: 'Descripción del registro 003', estado: 'Inactivo', fecha: '2026-03-10' },
  ];
...
<Layout>
  <Breadcrumb items={[{ label: nombreModulo }]} />

  <div class="page-header">
    <div>
      <h1 class="page-title">{nombreModulo}</h1>
      <p class="page-subtitle">Módulo de gestión</p>
    </div>
    {#if perms.agregar}
      <button class="btn btn-primary" onclick={() => { form = { nombre: '', descripcion: '', estado: 'Activo' }; modalAbierto = true; }}>
        + Nuevo Registro
      </button>
    {/if}
  </div>
  ...
  <div class="modal-overlay">
    <div class="modal">
      ...
      <div class="form-group">
        ...
        <input id="mnombre" type="text" class="form-control" placeholder="Nombre del registro" bind:value={form.nombre} />
      </div>
      <div class="form-group">
        <label for="mdesc">Descripción</label>
        <textarea id="mdesc" class="form-control" rows="3" placeholder="Descripción opcional" bind:value={form.descripcion} minlength="50"></textarea>
      </div>
      <div class="form-group">
        <label for="mestado">Estado</label>
        <select id="mestado" class="form-control" bind:value={form.estado}>
  ...
  <div class="modal-overlay">
    <div class="modal">
      ...
      <div class="form-group">
        ...
        <input id="menombre" type="text" class="form-control" bind:value={form.nombre} />
      </div>
      <div class="form-group">
        <label for="medesc">Descripción</label>
        <textarea id="medesc" class="form-control" rows="3" bind:value={form.descripcion} minlength="50"></textarea>
      </div>
      <div class="form-group">
        <label for="meestado">Estado</label>
        <select id="meestado" class="form-control" bind:value={form.estado}>
          <option>Activo</option>
          <option>Inactivo</option>
        </select>
      </div>
      ...
      <div class="detalle-grid">
        ...
        <div class="detalle-item">
          ...
          <span class="detalle-valor">
            <span class="badge {itemSeleccionado.estado === 'Activo' ? 'badge-success' : 'badge-danger'}">{itemSeleccionado.estado}</span>
          </span>
        </div>
...
{#if modalEliminar && itemSeleccionado && perms.eliminar}
  <div class="modal-overlay">
    <div class="modal modal-sm">
      ...
      <p class="eliminar-msg">¿Estás seguro de eliminar <strong>"{itemSeleccionado.nombre}"</strong>? Esta acción no se puede deshacer.</p>
      <div class="modal-footer">
